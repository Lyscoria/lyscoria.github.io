...
下面介绍本文的主要知识。正则语言是一类性质较好的语言，它有三种方式去描述，分别是确定性有限自动机（**D**eterministic **F**inite **A**utomaton）、非确定性有限自动机（**N**ondeterministic **F**inite **A**utomaton）和正则表达式（**R**egular **E**xpression），最终我们会证明这三种描述方式等价。正则语言的较重要的性质包括：
